...
//  protected final Logger logger = Logger.getLogger(getClass().getName());
  ...
  public Result send(Message message, String registrationId, int retries)
    ...
    do {
      ...
      log.debug("Attempt #{} to send message {} to regIds {}", attempt, message, registrationId);
...
//      logger.fine("GCM service is unavailable");
      log.error("GCM service is unavailable");
      return null;
    }
    if (status != 200) {
...
//              logger.warning("Received invalid second line from GCM: " + line);
              log.warn("Received invalid second line from GCM: {}", line);
            }
          }
          ...
          log.debug("Message created succesfully ({})", result);
  ...
  public MulticastResult send(Message message, List<String> regIds, int retries)
    ...
    do {
     ...
     /* if (logger.isLoggable(Level.FINE)) {
      ...
      log.debug("Attempt #{} to send message message to regIds {}", attempt, unsentRegIds);
...
//    HttpURLConnection conn = post(GCM_SEND_ENDPOINT, "application/json", requestBody);
    HttpURLConnection conn = this.post(GCM_SEND_ENDPOINT, "application/json;charset=UTF-8", requestBody);
...
//    byte[] bytes = body.getBytes();
    byte[] bytes = body.getBytes("UTF-8"); // jar로 실행하면 한글이 깨진다
